                ...
                const cl_event *event_list, cl_command_type type, cl_int *errcode_ret)
{
  ...
  do {
    ...
    if (queue == NULL) {
      ...
      if (!CL_EVENT_IS_USER(e)) {
        err = queue->device->api.event_create(queue->device, e);
        if (err != CL_SUCCESS)
          break;
      }
...
cl_event_exec(cl_event event, cl_int exec_to_status, cl_bool ignore_depends)
{
  ...
  for (s = cur_status - 1; s >= exec_to_status; s--) {
    ...
    ret = cl_enqueue_handle(event, s);

    if (ret != CL_SUCCESS) {
      cl_int set_ret;
      assert(ret < 0);
      CL_LOG_WARNING("Exec event %p error, type is %x, error status is %d",
                     event, event->event_type, ret);
      set_ret = cl_event_set_status(event, ret);
      assert(set_ret == CL_SUCCESS);
                                  ...
                                  const cl_event *event_wait_list, cl_bool is_barrier, cl_int *error)
{
  ...
  cl_event_enqueue_type eq_type = EnqueueMarker;
