...
specFile = open('builtin_vector_proto.def', 'r')
status = 0
header = open('ocl_stdlib.h', 'r')
newfile = []
...
for hline in header:
    ...
    if hline.strip() == '// ##BEGIN_BUILTIN_VECTOR##' :
        for line in specFile:
            ...
            if line[0] == '#':
                if line[1] == '#':
                    functionProto.append("//", line[2:])
                    functionProto.prefix=(line[2:].split())[0]
                continue

            functionProto.init_from_line(line)
            functionProto.gen_proto_str()
            functionProto.output(newfile)
        status = 1;
    if hline.strip() == '// ##END_BUILTIN_VECTOR##' :
        newfile.append(hline)
        status = 0;

specFile.close()
header.close()
# Update the new content to the header file.
newheader = open('ocl_stdlib.h', 'w')
for line in newfile:
